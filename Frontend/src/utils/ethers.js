...
export const withProviderRetry = async (fn, maxRetries = FALLBACK_URLS.length - 1) => {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[RPC] Call failed (attempt ${attempt + 1}), rotating provider:`, cleanMsg);
      rotateProvider();
    }
  }
  throw lastErr;
...
export const issueCertificate = async (certHash, name, course, ipfsHash) => {
  try {
    ...
    console.error("ISSUE ERROR:", (err?.message || "").replace(/https?:\/\/[^\s"'`]+/g, "[RPC_ENDPOINT]"));
    throw err;
  }
};
 ...
 * @param {number} chunkSize — blocks per request (default 100000 — safe for free-tier Infura)
 * @param {number} delayMs  — ms to wait between chunks
 */
export const queryFilterChunked = async (
  ...
  const isRateLimit = (err) => {
    ...
    try {
      if (body.includes("Too Many Requests") || body.includes("429")) return true;
    } catch (_) {}
  ...
  const isBlockRangeError = (err) => {
    ...
    const info = err?.info || {};
    const body = info?.responseBody || "";
    return msg.includes("block range") || msg.includes("limit") || msg.includes("exceeded") || msg.includes("too large") || body.includes("limit") || body.includes("range");
  };
